...
export class AppointmentStatisticsComponent {
        ...
        this.companyService.getCompanyForAdministrator(this.administratorId).subscribe(
          (company) => {
            ...
            if (this.company) {
              ...
              this.showTotalAppointmentsForLastFiveYears();
            }
          },
          (error) => {
  ...
  private showMonthlyAppointmentsForSelectedYear(selectedYear: number): void {
    const monthlyCountsObservables = this.months.map((month) => {
      console.log(`Fetching data for ${this.getMonthName(month)}, ${selectedYear}`);
      return this.service.getAppointmentCountsByMonth(this.company?.id!, selectedYear, month);
    });
    ...
    forkJoin(monthlyCountsObservables).subscribe(
      (monthlyCountsArray) => {
        ...
        this.drawMonthlyAppointmentsChart(this.months, monthlyCountsArray);
      },
      (error) => {
  ...
  private drawMonthlyAppointmentsChart(months: number[], countsArray: number[][]): void {
  ...
  onYearChange(): void {
    ...
    this.showMonthlyAppointmentsForSelectedYear(this.selectedYear!);
    this.showQuarterlyAppointments(this.selectedYear!);

    
   
  }

  private showQuarterlyAppointments(selectedYear: number): void {
    this.service.getAppointmentCountsByQuarter(this.company?.id!, selectedYear!).subscribe((counts) => {
      ...
      this.quarterlyChartData = [{ data: counts, label: 'Appointments by Quarter' }];
    });
  }
 ...
 /* onMonthChange(): void {
  ...
  private showTotalAppointmentsForLastFiveYears(): void {
    const currentYear = new Date().getFullYear();
    const lastFiveYears = Array.from({ length: 5 }, (_, i) => currentYear - i);
  
    const observables = lastFiveYears.map(year => {
      console.log(`Fetching data for year: ${year}`);
      return this.service.getAppointmentCountByYear(this.company?.id!, year);
    });
    ...
    forkJoin(observables).subscribe(
      (countsArray) => {
        ...
        this.drawTotalAppointmentsChart(lastFiveYears, countsArray);
      },
      (error) => {
  ...
  private drawTotalAppointmentsChart(years: number[], countsArray: number[]): void {
    countsArray.forEach((counts, index) => {
      const year = years[index];
      const chartLabel = `${year}`;
      const chartData = [{ data: [counts], label: `Appointments - ${year}` }];
